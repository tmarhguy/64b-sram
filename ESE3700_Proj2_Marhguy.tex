...
\begin{scope}[shift={(0,1.80)}]
  ...
  \fill[bar_del] (0,\yDel-\barH) rectangle (3.329*\sxFmax,\yDel+\barH);
  \node[anchor=west, font=\footnotesize\bfseries] at (0.5*\sxFmax+0.06,\yReq) {0.5\,GHz};
  \node[anchor=west, font=\footnotesize\bfseries, text=blue!55!black] at (3.329*\sxFmax+0.06,\yDel) {3.329\,GHz};
\end{scope}
...
Sustainable max CLK frequency $f_{\max}$ (back-to-back W/W/R/R) & $\ge 500$\,MHz & $3.329$\,GHz & $6.658\times$ & \texttt{find\_fmax.py} sweep, \S\ref{sec:fmax_sweep} \\
Min sustainable CLK period $T_{\min}$ & $\le 2$\,ns & $300.4$\,ps & $6.658\times$ & \texttt{find\_fmax.py} sweep, \S\ref{sec:fmax_sweep} \\
...
\Metric{Sustainable max CLK $f_{\max}$ (W/W/R/R sweep)}{3.329\,GHz @ 300.4\,ps} \quad
...
\caption{Representative \texttt{find\_fmax.py} transcript: best passing period
$0.300391$\,ns ($f_{\max}=3.329$\,GHz), $6.658\times$ vs.\ 500\,MHz spec,
$t_{\mathrm{CLK}\to D_{out}}=140.43$\,ps at $T_{\min}$, bisection tolerance
$\pm 5$\,ps, steady-state verify (8 W/W/R/R macros, 32 CLK cycles) PASS.
File: \texttt{media/terminal/fmax.png}.}
\label{fig:fmax_terminal}
\end{figure}

\HeadlineBox{\centering
\textbf{Sustainable max CLK} \quad
\Metric{$T_{\min}$}{300.4\,ps} \quad
\Metric{$f_{\max}$}{3.329\,GHz} \quad
\Metric{both readbacks}{PASS at $T_{\min}$} \quad
\Metric{margin vs.\ 500\,MHz spec}{$6.658\times$}}

The 300.4\,ps result is bounded by the precharge + non-overlap + write
settling envelope, not by the read access path; the single-read
$t_{\mathrm{CLK}\to D_{out}}$ measured at the same fast operating
point is $140.43\,$ps, essentially the same as the value extracted at
...
$f_{\max}=3.329\,$GHz is the sustainable cycle frequency that the
...
at $T_{\min}\approx 300.4\,$ps ($f_{\max}\approx 3.329\,$GHz). At the
same fast period the single-edge access $t_{\mathrm{CLK}\to D_{out}}$
holds at $140.43\,$ps, essentially the validation-deck value, so the
...
period $T_{\min}=300.4\,$ps from \S\ref{sec:fmax_sweep} as the delay
term gives
...
$f_{\max}\approx 3.329\,$GHz (period $T_{\min}=300.4\,$ps) measured by
...
Sustainable max CLK frequency $f_{\max}$ (W/W/R/R sweep) & 3.329\,GHz @ 300.4\,ps & \texttt{media/terminal/fmax.png}, \S\ref{sec:fmax_sweep} \\
\bottomrule
\end{tabularx}
...
\item \textbf{Synchronous, $f_{\max}\ge 500\,$MHz} --- sustainable
  ...
  sweep $f_{\max}\approx 3.329\,$GHz (min period $T_{\min}\approx
  300.4\,$ps), $\sim\!6.658\times$ over the spec floor; the
...
Sustainable max CLK frequency $f_{\max}$ (W/W/R/R sweep) & 3.329\,GHz @ 300.4\,ps \\
Equivalent frequency $f_{eq}=1/t_{\mathrm{CLK}\to D_{out}}$ (single-edge) & 7.00\,GHz \\
Specification frequency requirement & $\geq$ 500\,MHz (sustainable: $6.658\times$ margin) \\
...
\item \texttt{spice/find\_fmax.py} --- cycle-scaled $f_{\max}$ sweep
  ...
  \texttt{top.spi}; closes at $T_{\min}=300.4\,$ps
  ($f_{\max}=3.329\,$GHz), reproducing the sustainable max-CLK number
