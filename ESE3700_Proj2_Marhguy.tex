...
\begin{scope}[shift={(0,1.80)}]
  ...
  \fill[bar_del] (0,\yDel-\barH) rectangle (4.571*\sxFmax,\yDel+\barH);
  \node[anchor=west, font=\footnotesize\bfseries] at (0.5*\sxFmax+0.06,\yReq) {0.5\,GHz};
  \node[anchor=west, font=\footnotesize\bfseries, text=blue!55!black] at (4.571*\sxFmax+0.06,\yDel) {4.571\,GHz};
\end{scope}

% --- Panel B (top-right): f_eq (GHz), axis 0--10 ---
\begin{scope}[shift={(8,1.80)}]
  ...
  \fill[green!65!black, draw=black, line width=0.5pt] (0,\yDel-\barH) rectangle (9.037*\sxFeq,\yDel+\barH);
  \node[anchor=west, font=\footnotesize\bfseries] at (0.5*\sxFeq+0.06,\yReq) {0.5\,GHz};
  \node[anchor=west, font=\footnotesize\bfseries, text=green!60!black] at (9.037*\sxFeq+0.06,\yDel) {9.037\,GHz};
\end{scope}
...
\begin{scope}[shift={(0,-1.25)}]
  ...
  \fill[bar_del] (0,\yDel-\barH) rectangle (0.11065*\sxTd,\yDel+\barH);
  \node[anchor=east, font=\footnotesize\bfseries] at (2.0*\sxTd-0.06,\yReq) {2.000\,ns};
  \node[anchor=west, font=\footnotesize\bfseries, text=blue!55!black] at (0.11065*\sxTd+0.06,\yDel) {0.11065\,ns};
\end{scope}
...
sustainable $f_{\max}$ and equivalent $f_{eq}$ on a dedicated 0--10\,GHz
...
Sustainable max CLK frequency $f_{\max}$ (back-to-back W/W/R/R) & $\ge 500$\,MHz & $4.571$\,GHz & $9.14\times$ & \texttt{find\_fmax.py} sweep, \S\ref{sec:fmax_sweep} \\
Min sustainable CLK period $T_{\min}$ & $\le 2$\,ns & $218.75$\,ps & $9.14\times$ & \texttt{find\_fmax.py} sweep, \S\ref{sec:fmax_sweep} \\
Equivalent frequency $f_{eq}=1/t_{\mathrm{CLK}\to D_{out}}$ (single-edge latency) & --- & $9.037$\,GHz & --- & Fig.~\ref{fig:top_terminal_validation} \\
Top-level $t_{\mathrm{CLK}\to D_{out}}$ (single read access) & $<\!2$\,ns (cycle) & $110.65$\,ps & $18.07\times$ & Fig.~\ref{fig:top_terminal_validation} \\
...
Average power (top-level run)      & ---                & $21.367$\,\textmu W & ---            & Fig.~\ref{fig:top_terminal_validation} \\
\bottomrule
\end{tabularx}
...
$\mathrm{FOM} = 60 \cdot \mathrm{BitcellArea}\cdot \mathrm{Power}\cdot \mathrm{Delay}^2
              = 60 \cdot 8 \cdot (2.1367\!\times\!10^{-5}\,\mathrm{W})\cdot (110.65\,\mathrm{ps})^{2}
              \approx \mathbf{1.256\times 10^{-22}}.$
}
...
\item \textbf{Delay penalty of column muxing.} A column multiplexer
  ...
  FOM, even a $40$\,ps adder on a $\sim 110$\,ps access-time path inflates the
  squared-delay term by $\sim\!85\%$.
...
$f_{eq}\approx 9.037\,$GHz ($t_{\mathrm{CLK}\to D_{out}}\approx 110.65\,$ps); at the
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\linewidth,keepaspectratio]{media/terminal/final-terminal-out.png}
  \caption{Top-level extracted-netlist validation terminal output
  (\texttt{media/terminal/final-terminal-out.png}, generated
  by the \texttt{.control} block above). Both readbacks PASS,
  $t_{\mathrm{CLK}\rightarrow D_{out}}=1.1065{\times}10^{-10}\,$s,
  $I_{DD,\mathrm{avg}}=-2.1367{\times}10^{-5}\,$A
  ($P=21.367\,$\textmu W), and the equivalent frequency derived from the
  CLK-to-$D_{out}$ delay is $f_{eq}=9.037\,$GHz, $\sim\!18.07\times$ the
  500\,MHz spec floor.}
...
\Metric{$t_{\mathrm{CLK}\to D_{out}}$}{110.65\,ps} \quad
\Metric{$P_{\mathrm{avg}}$}{21.367\,\textmu W}\\[2pt]
\Metric{Sustainable max CLK $f_{\max}$ (W/W/R/R sweep)}{4.571\,GHz @ 218.75\,ps} \quad
\Metric{$f_{eq}=1/t_{\mathrm{CLK}\to D_{out}}$ (single-edge latency)}{9.037\,GHz}}
...
\emph{single-edge} CLK-to-$D_{out}$ access time (110.65\,ps) and
its inverse $f_{eq}=1/t_{\mathrm{CLK}\to D_{out}}\approx 9.037\,$GHz.
...
even though a single read edge resolves $D_{out}$ in $\sim\!110$\,ps.
...
search bracket was $[0.10, 2.00]\,$ns with a $5\,$ps tolerance.
For repeatable command-line runs with a formatted banner, optional
spinner, and automatic JSON capture, the repository includes the
wrapper script \texttt{scripts/run-find-fmax.sh}, which calls
\texttt{find\_fmax.py} with the same arguments used to produce the
reported evidence.
Figure~\ref{fig:fmax_search} summarizes the search loop;
Figure~\ref{fig:fmax_terminal} shows a saved \texttt{ngspice} transcript for the same flow.

\paragraph{Regression and screening rigor.}
Beyond the headline bisection, the repository includes a parallel driver
(\texttt{spice/parallel\_cell\_mix.py}) that composes dozens of \emph{integer}
$L=1$ / $L=2$ width recipes (per-library cell scales), runs
\texttt{find\_fmax.py} concurrently, and logs CSV outcomes. Recent sweeps varied
clock-local devices (\texttt{nor2}, \texttt{inv\_X8}, \texttt{inv}), replica
and decode stacks, precharge, flip-flop buffers, and dummy-column loads. On the
current \texttt{top.spi} export, every screened point returned the same
sustainable closure $T_{\min}=0.21875$\,ns ($f_{\max}=4.571$\,GHz), which is
strong evidence that the limiter is a shared timing envelope (precharge /
non-overlap / bitline budget) rather than any single tunable cell instance. The
flowchart in Fig.~\ref{fig:fmax_search} is the canonical depiction of the
acceptance test inside each of those jobs.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\linewidth,keepaspectratio]{media/terminal/fmax.png}
\caption{\texttt{find\_fmax.py} closure on the shipped \texttt{top.spi}: best passing period
$0.21875$\,ns ($f_{\max}=4.571$\,GHz), $9.14\times$ vs.\ 500\,MHz spec,
$t_{\mathrm{CLK}\to D_{out}}\approx 102$\,ps at $T_{\min}$, bisection tolerance
$\pm 5$\,ps, and steady-state verification PASS (32 W/W/R/R macros, 128 CLK cycles).
File: \texttt{media/terminal/fmax.png} (refresh after re-running the script for a matching screenshot).}
\label{fig:fmax_terminal}
\end{figure}

\HeadlineBox{\centering
\textbf{Sustainable max CLK} \quad
\Metric{$T_{\min}$}{218.75\,ps} \quad
\Metric{$f_{\max}$}{4.571\,GHz} \quad
\Metric{both readbacks}{PASS at $T_{\min}$} \quad
\Metric{margin vs.\ 500\,MHz spec}{$9.14\times$}}

The $T_{\min}\approx 218.75$\,ps result is bounded by the precharge + non-overlap + write
settling envelope, not by the raw read access path alone; the single-read
$t_{\mathrm{CLK}\to D_{out}}$ at the bisected fast period is on the order of
$100$--$110$\,ps, while the $500$\,MHz validation deck (\S\ref{sec:top}) reports
$110.65$\,ps on the address-0 read edge. The two frequency claims therefore describe
different things and are both kept in the report:
$f_{eq}\approx 9.037\,$GHz is the inverse of the read-access latency that
feeds the FOM \emph{delay} term (\S\ref{sec:fom}), while
$f_{\max}=4.571\,$GHz is the sustainable cycle frequency that the
...
measured delay is $t_{\mathrm{CLK}\rightarrow D_{out}}=110.65$\,ps.
...
at $T_{\min}\approx 218.75\,$ps ($f_{\max}\approx 4.571\,$GHz). At the
same fast period the single-edge access $t_{\mathrm{CLK}\to D_{out}}$
remains on the order of $100$--$110$\,ps, while the $500$\,MHz validation
deck reports $110.65$\,ps, so the
ratio $T_{\min}/t_{\mathrm{CLK}\to D_{out}}\approx 2.0$ is dominated
by the precharge + non-overlap envelope rather than the read access
path alone. This report uses the top-level measured access delay
($110.65$\,ps) as the delay term for the final Figure of Merit
...
\begin{figure}[H]
  ...
  measured on the address-0 read edge is $110.65\,$ps.}
...
$I_{DD,\mathrm{avg}}=21.367\,\textmu\mathrm{A}$ at
$V_{DD}=1\,\mathrm{V}$, corresponding to
$P_{\mathrm{avg}}=21.367\,\textmu\mathrm{W}$ (boxed below). The average is taken
over the full $12$\,ns \texttt{tran} window in \texttt{top.spi}, which includes
idle phases between stimulus edges, so it is a time-mean current rather than a
peak switching current.

\HeadlineBox{\centering
\Metric{$I_{DD,\mathrm{avg}}$ (12\,ns window)}{21.367\,\textmu A} \quad
\Metric{$V_{DD}$}{1.000\,V} \quad
\Metric{$P_{\mathrm{avg}}=V_{DD}\!\cdot\!I_{DD}$}{21.367\,\textmu W}}
...
\midrule
BL/BLB precharge recovery (4 columns, $\sim$ half swing each) & $\sim$ 16 fJ \\
Write driver active swing & $\sim$ 7 fJ \\
Cell storage-node flip energy ($\sim$ 4 cells flip per cycle in stress test) & $\sim$ 10 fJ \\
Decoder + clock + WL drivers + replica column + sized output buffers & $\sim$ 10 fJ \\
\midrule
\textbf{Total per cycle (at $f_{CLK}=500$\,MHz)} & $\sim$ \textbf{42.7 fJ} \\
\bottomrule
\end{tabularx}
\caption{Energy breakdown per cycle during the write-stress test
(scaled so the totals match the measured
$E_{cyc}=P_{\mathrm{avg}}/f_{CLK}=21.367\,\textmu\mathrm{W}/500\,\mathrm{MHz}\approx 42.7\,$fJ).
...
\pgfmathsetmacro{\k}{10.5/42.7}
\def\bh{0.52}
\pgfmathsetmacro{\xA}{16*\k}
\pgfmathsetmacro{\xB}{23*\k}
\pgfmathsetmacro{\xC}{33*\k}
\pgfmathsetmacro{\xD}{42.7*\k}
...
\node[anchor=south, font=\footnotesize\bfseries, inner sep=2.5pt] at ({8*\k},\bh) {16};
\node[anchor=south, font=\footnotesize\bfseries, inner sep=2.5pt] at ({19.5*\k},\bh) {7};
\node[anchor=south, font=\footnotesize\bfseries, inner sep=2.5pt] at ({28*\k},\bh) {10};
\node[anchor=south, font=\footnotesize\bfseries, inner sep=2.5pt] at ({37.85*\k},\bh) {10};
\node[anchor=west, font=\small\bfseries] at (0,{\bh+0.62}) {Qualitative energy per cycle (fJ)};
\node[anchor=west, font=\scriptsize, text=black!58] at (0,-0.34)
  {Segment width $\propto$ energy; total $\sim$42.7\,fJ at $f_{CLK}=500$\,MHz (Tab.~above).};
\end{tikzpicture}\\[4pt]
...
\end{tabular}\par}
{\centering\scriptsize\textcolor{black!55}{Approx.\ shares: $\sim$37\%, $\sim$16\%, $\sim$23\%, $\sim$23\%.
...
\item $\mathrm{Power} = 21.367\,$\textmu W $= 2.1367\times 10^{-5}\,$W (top-level measured).
\item $\mathrm{Delay} = 110.65\,$ps (top-level measured); $\mathrm{Delay}^2 = 1.224\times 10^{-20}\,$s$^{2}$.
\end{itemize}

\[
  \mathrm{FOM} = 60 \cdot 8 \cdot 2.1367\times 10^{-5} \cdot \left(1.1065\times 10^{-10}\right)^{2}
              \approx 1.256\times 10^{-22}.
\]

\textbf{Access-time vs.\ cycle-time FOM.} The number above uses the
single-edge read access $t_{\mathrm{CLK}\to D_{out}}=110.65\,$ps as
...
period $T_{\min}=218.75\,$ps from \S\ref{sec:fmax_sweep} as the delay
term gives
$60\cdot 8\cdot 2.1367\!\times\!10^{-5}\cdot(2.1875\!\times\!10^{-10})^{2}
\approx 4.91\!\times\!10^{-22}$. Both formulations meet the spec
floor by large margins; this report carries the access-time value
forward as the headline FOM.

\textbf{What the scalar FOM is (and is not).} The project formula
multiplies declared inputs: bitcell width sum, measured average supply
power, and a delay. Here $P_{\mathrm{avg}}$ is the time-mean
$V_{DD}\cdot I_{DD}$ over the $12$\,ns top-level validation transient
at $f_{\mathrm{CLK}}=500$\,MHz (\S\ref{sec:power}), while
$D$ is the single-edge CLK-to-$D_{out}$ latency from the same deck
(\S\ref{sec:top}). Sustainable max CLK
$f_{\max}\approx 4.571$\,GHz is established separately by the
\texttt{find\_fmax.py} sweep (\S\ref{sec:fmax_sweep}) and is not used
as the delay factor in the headline FOM. The headline value should
therefore be read as the rubric composite from those cited
measurements---not as a literal single-point energy--delay product at
sustained $f_{\max}$, and not as average power at maximum switching
throughput (which would require a dedicated long transient or average
current measurement matched to that activity). The alternate
cycle-time FOM above likewise keeps the same $P_{\mathrm{avg}}$ for
comparison only; remeasuring $P$ under a $T_{\min}$ macro would be
needed for a fully self-consistent throughput-matched composite.
...
\midrule
\textbf{This design (6T 2-1-1, no mux, StrongARM, replica)} & 8 & 111 & 21.367 & \textbf{1.00} \\
...
$t_{\mathrm{CLK}\to D_{out}}=110.65\,$ps from the validation
\texttt{.control} block (whose inverse is the
$f_{eq}\approx 9.037\,$GHz figure that feeds the FOM delay term),
and the \emph{sustainable max CLK frequency}
$f_{\max}\approx 4.571\,$GHz (period $T_{\min}=218.75\,$ps) measured by
the cycle-stepped sweep in \S\ref{sec:fmax_sweep} via
\texttt{spice/find\_fmax.py} (or the convenience harness
\texttt{scripts/run-find-fmax.sh}). The latter is the spec's ``max CLK
...
Parallel integer-width sweeps (\texttt{parallel\_cell\_mix.py}, logged CSVs under
\texttt{spice/}) were additionally used as a regression screen; on the current
extracted deck they reproduce the same $T_{\min}$ closure reported above.
...
\item \textbf{Mid-cycle input change.} During $\Phi_2$ of a write
  ...
  input registers.
\item \textbf{Sustainable-$f_{\max}$ sweep (W/W/R/R).} The spec-defined
  max clock frequency is measured using the cycle-scaled W/W/R/R macro
  sweep in \S\ref{sec:fmax_sweep}. The script \texttt{spice/find\_fmax.py}
  binary-searches the minimum CLK period, scaling \emph{all} PWL sources
  each trial, and accepts a candidate period only if both readbacks
  (\texttt{0x5} and \texttt{0xA}) pass at a 0.5\,V threshold. The optional
  post-search steady verify runs many consecutive macros at $T_{\min}$ to
  strengthen the sustained claim.
\end{enumerate}
...
Top-level $t_{\mathrm{CLK}\to D_{out}}$ (single-edge access) & 110.65\,ps & \texttt{media/terminal/final-terminal-out.png} \\
Top-level $I_{DD,\mathrm{avg}}$ / $P_{\mathrm{avg}}$ & 21.367\,\textmu A / 21.367\,\textmu W & \texttt{media/terminal/final-terminal-out.png} \\
Top-level readback @ 5\,ns / @ 7\,ns & $\mathtt{0x5}$ / $\mathtt{0xA}$ (PASS) & \texttt{media/terminal/final-terminal-out.png} \\
Delay-derived equivalent frequency $f_{eq}=1/t_{\mathrm{CLK}\to D_{out}}$ & 9.037\,GHz @ 110.65\,ps & \texttt{media/terminal/final-terminal-out.png} \\
Sustainable max CLK frequency $f_{\max}$ (W/W/R/R sweep) & 4.571\,GHz @ 218.75\,ps & \texttt{media/terminal/fmax.png}, \S\ref{sec:fmax_sweep} \\
\bottomrule
\end{tabularx}
...
\item \textbf{Synchronous, $f_{\max}\ge 500\,$MHz} --- sustainable
  ...
  sweep $f_{\max}\approx 4.571\,$GHz (min period $T_{\min}\approx
  218.75\,$ps), $\sim\!9.14\times$ over the spec floor; the
  delay-derived $f_{eq}=1/t_{\mathrm{CLK}\to D_{out}}\approx 9.037\,$GHz
...
\item \textbf{Read or write completes within one CLK period} ---
  $t_{\mathrm{CLK}\to D_{out}}=110.65\,$ps single-edge access;
  cell-level write times $9.87/21.8\,$ps; spec is met at any cycle
  $T\ge T_{\min}=218.75\,$ps.
...
\item \textbf{Power measurement methodology} --- average $V_{DD}$
  ...
  (\S\ref{sec:power}); reported $P_{\mathrm{avg}}=21.367\,$\textmu W.
\item \textbf{FOM reported} ---
  $\mathrm{FOM}=60\!\cdot\!8\!\cdot\!P\!\cdot\!D^{2}\approx
  1.256\!\times\!10^{-22}$ (\S\ref{sec:fom}).
\end{itemize}
...
Minimum CLK period (with margin) & 219\,ps \\
Measured CLK-to-$D_{out}$ (single-edge access) & 110.65\,ps \\
Sustainable max CLK frequency $f_{\max}$ (W/W/R/R sweep) & 4.571\,GHz @ 218.75\,ps \\
Equivalent frequency $f_{eq}=1/t_{\mathrm{CLK}\to D_{out}}$ (single-edge) & 9.037\,GHz \\
Specification frequency requirement & $\geq$ 500\,MHz (sustainable: $9.14\times$ margin) \\
Average power, top-level run & 21.367\,\textmu W \\
...
\textbf{FOM} $= 60\cdot A\cdot P\cdot D^2$ & \textbf{$\approx 1.256\times 10^{-22}$} \\
\bottomrule
\end{tabularx}
...
\item \texttt{spice/find\_fmax.py} --- cycle-scaled $f_{\max}$ sweep
  ...
  \texttt{top.spi}; closes at $T_{\min}=218.75\,$ps
  ($f_{\max}=4.571\,$GHz), reproducing the sustainable max-CLK number
